...
class VerificationParameters(BaseModel):
    ...
    def __init__(self, **data):
        ...
        self.data_dir = self.data_dir / self.input_product
    ...
    def check_input_product(self) -> "VerificationParameters":
        if self.input_product not in ["s1", "hls"]:
            raise ValueError(f'Input product must be in {", ".join(INPUT_PRODUCTS)}')
    ...
    def check_rel_dswx_db_dir_exists(self) -> "VerificationParameters":
        if self.rel_dswx_db_dir_path is not None:
            if not self.rel_dswx_db_dir_path.exists():
                raise ValueError(
                    "The specified location of the local DSWx DB does not exist; "
    ...
    def check_if_local_data_exist(self) -> "VerificationParameters":
        if self.rel_dswx_db_dir_path is not None:
            ...
            if not any([p.exists() for p in val_paths]):
                raise ValueError(
                    "The necessary validation datasets do not exist in the local directory"
            ...
            if not any([p.exists() for p in dswx_paths_path]):
                raise ValueError(
                    "The necessary DSWx paths were not found; please verify you specified the relative "
